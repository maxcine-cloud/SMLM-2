...
from .sequence_derived_features import *
...
class BertCatPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(int(config.hidden_size*2)+47, int(config.hidden_size*2)+47)
        self.activation = ACT2FN["gelu"]
        # self.dense2 = nn.Linear(config.hidden_size, config.hidden_size)
    def forward(self, hidden_states,hidden_states2,hidden_states3):
        ...
        catpooler=torch.cat([hidden_states,hidden_states2,hidden_states3],dim=1)
        catpooler = self.dense(catpooler)
        catpooler = self.activation(catpooler)
        # catpooler=hidden_states+hidden_states2
        return catpooler


class BertPre(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(int(16*101), int(config.hidden_size/2))
...
#            head_mask = [None] * self.config.num_hidden_layers
        ...
        HCF = HCF.view(sequence_output.shape[0],-1)
        catpooler_output=self.catpooler(alted_output,res_blok,HCF)
...
class ResNet_seq(nn.Module):
    def __init__(self):
        super().__init__()
        self.blk1 = ResBlk(6, 128, stride=1) # [b, 64, h, w] => [b, 128, h ,w]
        self.blk2 = ResBlk(128, 256, stride=2)
        self.avgPool=nn.AdaptiveAvgPool2d([1, 1])
        self.outlayer = nn.Linear(256*1*1, 768) 
    def forward(self, x):
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(int(config.hidden_size*2)+47, 2)
...
def calc_loss(y_pred, labels, enc_mean, enc_std, beta=1e-3):
    ...
    ce = nn.CrossEntropyLoss(reduction='sum')(y_pred,labels)
